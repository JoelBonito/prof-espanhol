...
export function LessonSummaryCard({
  ...
  return (
    <Card className="p-5 md:p-6">
      ...
      <h1 className="font-display text-2xl text-text-primary font-bold">{lesson.title}</h1>
      <p className="font-body text-text-muted text-sm mt-1">{lesson.topic}</p>

      <div className="mt-4">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-xs font-semibold text-primary-500">Progresso do módulo</span>
          <span className="text-xs font-bold text-text-secondary">{Math.round(moduleProgressPercent)}%</span>
        </div>
