  ...
  const renderArea = (area: AdaptationArea) => {
    ...
    return (
      <Card key={area} className="p-5 md:p-6">
        <h3 className="font-display text-xl font-semibold text-neutral-900 mb-3">{AREA_LABELS[area]}</h3>
        <div className="space-y-3">
          {areaEntries
            .slice()
            .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
            .map((entry, index) => (
              <div key={`${entry.date}-${entry.reason}-${index}`} className="relative rounded-xl border border-neutral-200 p-4 bg-white">
                ...
                <p className="font-body text-sm text-neutral-800">{adjustmentText(entry.adjustment)}</p>
                <p className="font-body text-xs text-neutral-500 mt-1">Motivo: {entry.reason}</p>
